...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
		...
		pub fn transfer_claim(
            ...
            let (owner, _blocknumber) = Proofs::<T>::get(&bounded_claim).ok_or(Error::<T>::ClaimNotExist)?;

			// ensure!(!Proofs::<T>::contains_key(&proof), Error::<T>::NoSuchProof);
			// let (owner, block_number) = Proofs::<T>::get(&proof).ok_or(Error::<T>::NoSuchProof)?;
			ensure!(owner == sender, Error::<T>::NotClaimOwner);

			Proofs::<T>::insert(&bounded_claim, (receiver.clone(), frame_system::Pallet::<T>::block_number()));


			Self::deposit_event(Event::ClaimTransferred(receiver, claim));
